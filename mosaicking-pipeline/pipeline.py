...
from eogrow.tasks.features import (
...
from eogrow.types import Feature, FeatureSpec, PatchList, TimePeriod
from eogrow.utils.filter import get_patches_with_missing_features
from eogrow.utils.validators import field_validator, optional_field_validator, parse_dtype, parse_time_period
...
class MosaickingPipeline(Pipeline):
    """A pipeline to calculate and prepare features for ML"""

    _NDI_FEATURE = FeatureType.DATA, "_NDI_FEATURE_OF_MOSAICKING_PIPELINE"  # used by MaxNDVIMosaickingTask
    ...
    def get_data_preparation_node(self) -> EONode:
        ...
        if filtering_config.cloud_mask_feature_name:
            zip_masks_task = JoinMasksTask(
                [self._get_valid_data_feature(), (FeatureType.MASK, filtering_config.cloud_mask_feature_name)],
        ...
        if self.config.mosaicking.max_ndi_indices:
            ...
            end_node = EONode(ndi_task, inputs=[end_node])
